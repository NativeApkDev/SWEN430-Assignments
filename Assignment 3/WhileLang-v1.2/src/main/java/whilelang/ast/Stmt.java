...
public interface Stmt extends SyntacticElement {
	...
	/**
	 * Represents a delete statement whose body is a single expression which
	 * evaluates to a reference.  As an example:
	 *
	 * <pre>
	 * &int p = new 1
	 * ...
	 ...
	 *
	 */
	public static final class Delete extends SyntacticElement.Impl implements Stmt {
		/**
		 * The expression which, when evaluated, determines whether or not the assertion
		 * holds.
		 */
		...
		/**
		 * Construct a delete statment from a given list of attributes.
		 *
		 * @param expr       May not be null.
		 * @param attributes
		 */
		public Delete(Expr expr, Attribute... attributes) {
			super(attributes);
			this.expr = expr;
		}
		 ...
		 * @param attributes
		 */
		public Delete(Expr expr, Collection<Attribute> attributes) {
			super(attributes);
			this.expr = expr;
		}
