...
public interface Stmt extends SyntacticElement {
	// Creating a class representing 'throw' statements.
	...
	public static final class Throw extends SyntacticElement.Impl implements Stmt {
		...
		// 1. The expression after the 'throw' statement
		private final Expr expr;

		// 2. The type of the throw statement. This can be null if the throw expression does not have a type.
		private Type type;

		// Creating main constructor for Throw class
		public Throw(Expr expr, Attribute... attributes) {
			super(attributes);
			this.expr = expr;
		}

		// Creating second constructor for Throw class
		public Throw(Expr expr, Collection<Attribute> attributes) {
			super(attributes);
			this.expr = expr;
		}

		// Create constructors for Throw class which initialise the type
		public Throw(Expr expr, Type type, Attribute... attributes) {
			super(attributes);
			this.expr = expr;
			this.type = type;
		}

		public Throw(Expr expr, Type type, Collection<Attribute> attributes) {
			super(attributes);
			this.expr = expr;
			this.type = type;
		}
		...
		// Getters and setters for Throw class
		public Expr getExpr() {
			return expr;
		}
	...
	// Creating a class representing 'try-catch' statements.
	// This class is inspired by the following source.
	// https://github.com/kianyewest/SWEN430/tree/master/swen430as1
	...
	public static final class TryCatch extends SyntacticElement.Impl implements Stmt {
		...
		// 1. The body of the "try" statement. The body can contain zero or more statements.
		private final List<Stmt> body;

		// 2. A list of catch statements in the try-catch clause.
		private final List<Catch> catchStatements;

		// Creating first constructor of TryCatch class
		public TryCatch(List<Stmt> body, List<Catch> catchStatements,
						Collection<Attribute> attributes) {
			super(attributes);
			this.body = new ArrayList<>(body);
			this.catchStatements = new ArrayList<>(catchStatements);
		}

		// Creating second constructor of TryCatch class
		public TryCatch(List<Stmt> body, List<Catch> catchStatements,
		...
		// Creating getters for TryCatch class

		public List<Stmt> getBody() {
			return body;
		}

		public List<Catch> getCatchStatements() {
			return catchStatements;
		}
	}

	// Creating a class representing 'catch' statements.
	// This class is inspired by the following source.
	// https://github.com/kianyewest/SWEN430/tree/master/swen430as1
	...
	public static final class Catch extends SyntacticElement.Impl implements Stmt {
		...
		// 1. The variable declaration containing the type of the caught variable and its name
		private final VariableDeclaration caughtVariable;

		// 2. The body of the "catch" statement. The body can contain zero or more statements.
		private final List<Stmt> body;

		// Creating first constructor of Catch class
		public Catch(VariableDeclaration caughtVariable, List<Stmt> body,
					 Collection<Attribute> attributes) {
			super(attributes);
		...
		// Creating second constructor of Catch class
		public Catch(VariableDeclaration caughtVariable, List<Stmt> body,
					 Attribute... attributes) {
			super(attributes);
			this.caughtVariable = caughtVariable;
		...
		// Creating getters of Catch class

		public VariableDeclaration getCaughtVariable() {
		...
		// Creating methods to add an element to 'body'
		public void addStmt(int index, Stmt stmt) {
			body.add(index, stmt);
		}
		...
		public void addStmtAtFirst(Stmt stmt) {
			addStmt(0, stmt);
		}

		// Creating a method to remove an element from 'body'
		public boolean removeStmt(Stmt stmt) {
			return body.remove(stmt);
		}
	}
	...
	public static final class Assert extends SyntacticElement.Impl implements
		...
		/**
		 * Get the expression whose value is to be asserted.
		 *
	...
	public static final class DoWhile extends SyntacticElement.Impl implements Stmt {
		...
		/**
		 * Construct a DoWhile statement from a given condition and body of statements.
		 *
		 * @param body       non-null collection which contains zero or more statements.
		 * @param condition  non-null expression.
		 * @param attributes
		 */
		public DoWhile(Collection<Stmt> body, Expr condition, Attribute... attributes) {
			super(attributes);
		 ...
		 * @param attributes
		 */
		public DoWhile(Collection<Stmt> body, Expr condition, Collection<Attribute> attributes) {
			super(attributes);
			this.condition = condition;
			this.body = new ArrayList<>(body);
		}
	...
	// Creating a class which deals with for each statements in While programming language.
	// This class is inspired by the following source.
	// https://github.com/kianyewest/SWEN430/tree/master/swen430as1
	...
	public static final class ForEach extends SyntacticElement.Impl implements Stmt {
		...
		// 1. The loop index variable
		private final VariableDeclaration declaration;

		// 2. An expression representing a collection of values to iterate through
		private final Expr values;

		// 3. The body of the loop. It can contain zero or more statements.
		private final ArrayList<Stmt> body;

		// Creating a constructor for ForEach class
		public ForEach(VariableDeclaration declaration, Expr values, Collection<Stmt> body,
					   Collection<Attribute> attributes) {
			super(attributes);
			this.declaration = declaration;
			this.values = values;
		...
		// Creating second constructor for ForEach class
		public ForEach(VariableDeclaration declaration, Expr values, Collection<Stmt> body,
					   Attribute... attributes) {
			super(attributes);
			this.declaration = declaration;
	...
	public static final class IfElse extends SyntacticElement.Impl implements
		...
		public IfElse(Expr condition, List<Stmt> trueBranch,
			...
			super(attributes);
			this.condition = condition;
			this.trueBranch = new ArrayList<>(trueBranch);
			this.falseBranch = new ArrayList<>(falseBranch);
		}
