...
public class TypeChecker {
	...
	public Type check(Expr.Binary expr, Map<String,Type> environment) {
		...
		switch(expr.getOp()) {
			...
			case REM:
				...
				checkInstanceOf(leftType,expr.getLhs(),Type.Int.class,Type.RealNumber.class);
				checkInstanceOf(rightType,expr.getRhs(),Type.Int.class,Type.RealNumber.class);
				return leftType;
			case EQ:
			case NEQ:
	...
	public Type check(Expr.IndexOf expr, Map<String, Type> environment) {
		...
		// Make sure index has integer type
		checkInstanceOf(indexType, expr.getIndex(), Type.Int.class,Type.RealNumber.class);
	...
	public Type check(Expr.ArrayGenerator expr, Map<String, Type> environment) {
		...
		checkInstanceOf(size,expr.getSize(),Type.Int.class,Type.RealNumber.class);
	...
	public Type check(Expr.Unary expr, Map<String,Type> environment) {
		...
		switch(expr.getOp()) {
			case NEG:
				checkInstanceOf(type,expr.getExpr(),Type.Int.class,Type.RealNumber.class);
				return type;
			case NOT:
	...
	public Type checkInstanceOf(Type type,
		...
		for (Class<?> instance : instances) {
			...
			if (instance.getName().endsWith("Bool")) {
				...
				msg += "int";
			} else if (instance.getName().endsWith("RealNumber")) {
				msg += "real";
	...
	public Type[] expandAtoms(Type type) {
		if (type instanceof Type.Void || type instanceof Type.Null || type instanceof Type.Bool || type instanceof Type.Int) {
			...
			if (types.containsKey(t.getName())) {
				...
				syntaxError("unknown type encountered: " + type, file.filename, type);
				return null;
			}
		}
	}
