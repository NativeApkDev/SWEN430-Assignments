...
public class DefiniteAssignment {
	...
	public ControlFlow check(List<Stmt> statements, Defs environment) {
		...
		for (Stmt s : statements) {
			...
			continueEnvironment = join(continueEnvironment, nf.continueEnvironment);
		}
		return new ControlFlow(nextEnvironment, breakEnvironment, continueEnvironment);
	}

	/**
	 * Check that all variables used in a given statement are definitely assigned.
	 * Furthermore, update the set of definitely assigned variables to include any
	 * which are definitely assigned after this statement.
	 *
	 * @param statement   The statement to check.
	 * @param environment The set of variables which are definitely assigned.
	 * @return The updated set of variables which are now definitely assigned, or
	 *         null if the method has terminated.
	 */
	public ControlFlow check(Stmt stmt, Defs environment) {
		if (stmt instanceof Stmt.Assert) {
			...
			return new ControlFlow(environment, null, null);
	...
	public ControlFlow check(Stmt.Break stmt, Defs environment) {
		...
		return new ControlFlow(null, environment, null);
	}

	public ControlFlow check(Stmt.Continue stmt, Defs environment) {
		// Here we just move the current environment into the "continue"
		// control-flow position.
		return new ControlFlow(null, null, environment);
	}

	public ControlFlow check(Stmt.DoWhile stmt, Defs environment) {
		ControlFlow b = check(stmt.getBody(), environment);
		// Determine environment going into condition. This does not include the break
		// environment as, in such case, the condition is not executed.
		Defs continueEnvironment = join(b.nextEnvironment, b.continueEnvironment);
		// Determine environment following the loop which also must include the break
		// environment.
		Defs nextEnvironment = join(b.breakEnvironment, continueEnvironment);
		//
		check(stmt.getCondition(), continueEnvironment);
		//
		return new ControlFlow(nextEnvironment, null, null);
	}
	...
	public ControlFlow check(Stmt.Return stmt, Defs environment) {
		...
		return new ControlFlow(null, null, null);
	}
	...
	public ControlFlow check(Stmt.Switch stmt, Defs environment) {
		...
		return new ControlFlow(breakEnvironment, null, null);
	}
	...
	public void check(Expr expr, Defs environment) {
		if (expr instanceof Expr.Binary) {
			...
			check((Expr.Literal) expr, environment);
	...
	public void check(Expr.Literal expr, Defs environment) {
	...
	private class ControlFlow {
		...
		/**
		 * The set of definitely assigned variables on this path which are on the
		 * control-flow path caused by a contrinue statement.
		 */
		public final Defs continueEnvironment;

		public ControlFlow(Defs nextEnvironment, Defs breakEnvironment, Defs continueEnvironment) {
			this.nextEnvironment = nextEnvironment;
			this.breakEnvironment = breakEnvironment;
			this.continueEnvironment = continueEnvironment;
		}
		...
		public ControlFlow merge(ControlFlow other) {
			...
			Defs c = join(continueEnvironment, other.continueEnvironment);
			return new ControlFlow(n, b, c);
		}
	}
