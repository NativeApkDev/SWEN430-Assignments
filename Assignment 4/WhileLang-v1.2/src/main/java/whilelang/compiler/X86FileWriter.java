 ...
 * 1. https://github.com/kianyewest/SWEN430/tree/master/swen430as4
 * */
...
public class X86FileWriter {
	...
	public void translate(Stmt statement, Context context) {
		if (statement instanceof Stmt.Assert) {
			...
			translateDoWhile((Stmt.DoWhile) statement, context);
		} else if (statement instanceof Stmt.Delete) {
			translateDelete((Stmt.Delete) statement, context);
		} else {
			throw new IllegalArgumentException("Unknown statement encountered: " + statement);
		}
	}

	// Translating "Stmt.Delete" statement into machine code

	public void translateDelete(Stmt.Delete statement, Context context) {
	...
	public void translateAssign(Stmt.Assign statement, Context context) {
		...
		if (lhs instanceof Expr.Variable) {
			...
			translate(rhs, payloadField, context);
		}
	}
	...
	public void translateBreak(Stmt.Break statement, Context context) {
		...
		// Getting the list of instructions in the context 'context'
		List<Instruction> instructions = context.instructions();

		// Adding an instruction to jump to the address of the most recent 'break' keyword
		String mostRecentBreakLabel = context.viewMostRecentBreakLabel();
		Instruction jumpToBreak = new Instruction.Addr(Instruction.AddrOp.jmp, mostRecentBreakLabel);
		instructions.add(jumpToBreak);
	}
	...
	public void translateContinue(Stmt.Continue statement, Context context) {
		...
		// Adding an instruction to jump to the address of the most recent 'continue' keyword
		String mostRecentContinueLabel = context.viewMostRecentContinueLabel();
		Instruction jumpToContinue = new Instruction.Addr(Instruction.AddrOp.jmp, mostRecentContinueLabel);
		instructions.add(jumpToContinue);
	}
	...
	// https://github.com/kianyewest/SWEN430/tree/master/swen430as4
	public void translateFor(Stmt.For statement, Context context) {
		...
		// Construct label for the end of an iteration of the for loop
		...
		// Add an instruction for the continue label
		Instruction continueLabelInstruction = new Instruction.Label(continueLabel);
		instructions.add(continueLabelInstruction);
		...
		// Removing labels since we have finished iterating through the for loop
		// 1. Removing the break label
		context.getMostRecentBreakLabel();

		// 2. Removing the continue label
		context.getMostRecentContinueLabel();
	}
	...
	/**
	 * Translating a do-while statement in the While language into a sequence of machine instructions.
	 * */

	// This method is inspired by the following source.
	// https://github.com/kianyewest/SWEN430/tree/master/swen430as4
	public void translateDoWhile(Stmt.DoWhile statement, Context context) {
		// Getting a list of instructions in the context 'context'
		List<Instruction> instructions = context.instructions();
		...
		// Translating the body of the do-while loop
		List<Stmt> doWhileLoopBody = statement.getBody();
		for (Stmt bodyStmt : doWhileLoopBody) {
			translate(bodyStmt, context);
		}

		// Initialising an instruction to start the loop and then add it to the list 'instructions'
		Instruction startLoop = new Instruction.Label(headerLabel);
		instructions.add(startLoop);
		// Translating the condition of the while loop
		Expr condition = statement.getCondition();
		translateCondition(condition, breakLabel, context);
		// Translating the body of the while loop again
		for (Stmt currStmt : doWhileLoopBody) {
			if (currStmt instanceof Stmt.VariableDeclaration) {
				...
				if(rhs != null) {
					...
					Stmt.Assign assignStmt = new Stmt.Assign(lhs, rhs);
					translateAssign(assignStmt, context);
				}
			}
			else {
				translate(currStmt, context);
			}
		}

		// Initialising an instruction to jump to the start of the loop if the condition is true.
		Instruction jumpToStart = new Instruction.Addr(Instruction.AddrOp.jmp, headerLabel);

		// Initialising an instruction to end the loop
		Instruction endLoop = new Instruction.Label(breakLabel);

		// Adding the recently initialised instructions to the list 'instructions'
		List<Instruction> toBeAdded = Arrays.asList(jumpToStart, endLoop);
		instructions.addAll(toBeAdded);

		// Removing the created labels related to 'break' and 'continue'.
		context.getMostRecentBreakLabel();
		context.getMostRecentContinueLabel();
	}
	...
	public void translateWhile(Stmt.While statement, Context context) {
		...
		Expr condition = statement.getCondition();
		translateCondition(condition, breakLabel, context);
		// Translating the body of the while loop
		List<Stmt> whileLoopBody = statement.getBody();
		translate(whileLoopBody, context);
	...
	public void translateSwitch(Stmt.Switch statement, Context context) {
		// Getting a list of instructions in the context 'context'
		List<Instruction> instructions = context.instructions();
		// Translate the expression we are switching on
		Expr lhsExpr = statement.getExpr();
		// Allocating two temporary locations for equality comparisons
		RegisterLocation[] tmps = allocateRegisterLocations(context, lhsExpr, statement.getExpr());
		...
		// Placing the result onto the target register.
		translate(lhsExpr, tmps[0], context);
		// Getting the type of the left-hand side expression
		Type lhsType = unwrap(lhsExpr.attribute(Attribute.Type.class).type);
		...
		// Initialising a label for the next body
		String nextBody = freshLabel();

		int i = 0; // initial value
		List<Stmt.Case> cases = statement.getCases();
		while (i < cases.size()) {
			Stmt.Case c = cases.get(i);
			String nextLabel = freshLabel();
			Expr constant = c.getValue();
			if (constant != null) {
				// Adding missing attributes to 'c.getValue()'
				c.getValue().attributes().add(lhsExpr.attribute(Attribute.Type.class));

				// Not a default block
				translate(c.getValue(), tmps[1], context);

				// Getting the type of the right-hand side expression
				Type rhsType = unwrap(c.getValue().attribute(Attribute.Type.class).type);

				// Checking for equality between both sides of the expression
				// 1. If neither sides are primitive types
				if (!isPrimitive(lhsType) && !isPrimitive(rhsType)) {
					compoundEquality(false, tmps[1], tmps[0], nextLabel, context);
				}
				// 2. Else if both sides are primitive types
				else if (isPrimitive(lhsType) && isPrimitive(rhsType)) {
					bitwiseEquality(false, tmps[0], tmps[1], nextLabel, context);
				}
				// 3. Else if neither sides are primitive types
				else {
					// Add an instruction to jump to the next label since the equality comparision will obviously
					// return false due to different types being compared
					Instruction jumpToNextLabel = new Instruction.Addr(Instruction.AddrOp.jmp, nextLabel);
					instructions.add(jumpToNextLabel);
				}
			}
			// DONEFIXME: need to handle break and continue statements!
			Instruction nextBodyLabel = new Instruction.Label(nextBody);
			instructions.add(nextBodyLabel);
			translate(c.getBody(), context);

			// Since the condition is true, go directly to the body of the switch statement

			// Check if we are not yet at the last case in the switch case statement
			if (statement.getCases().size() > (i + 1) && !c.isDefault()) {
				// Initialising a new label for the next body
				nextBody = freshLabel();
		...
		// Remove the break label from the stack in context 'context'
		context.getMostRecentBreakLabel();
	}
	...
	public void translateCondition(Expr e, String falseLabel, Context context) {
		...
		if (e instanceof Expr.Unary) {
			...
			switch (b.getOp()) {
			case AND:
				// Getting both sides of the expression and translating them into machine code
				Expr lhs = b.getLhs();
				Expr rhs = b.getRhs();
				translateCondition(lhs, falseLabel, context);
				translateCondition(rhs, falseLabel, context);
				break;
			case OR:
	...
	public void translateEqualityCondition(Expr.Binary e, String falseLabel, Context context) {
		...
		if (isPrimitive(lhs_t) && isPrimitive(rhs_t)) {
			...
			// Perform a compound comparison of the two data chunks
	...
	public MemoryLocation translate(LVal lval, Context context) {
		if (lval instanceof Expr.Variable) {
			...
			return translateArrayLVal((Expr.ArrayAccess) lval, context);
		}
	}
	...
	// Creating a method to translate array lval.
	// This method is inspired by the following source.
	// https://github.com/kianyewest/SWEN430/tree/master/swen430as4
	public MemoryLocation translateArrayLVal(Expr.ArrayAccess lval, Context context) {
		// DONEFIXME: you will need to implement this
		Expr source = lval.getSource();
		Expr index = lval.getIndex();
		return indexHelper(source, index, context);
	}
	...
	public void translate(Expr expression, Location target, Context context) {
		if (expression instanceof Expr.ArrayGenerator) {
			...
			translateArrayAccess((Expr.ArrayAccess) expression, target, context);
			...
			translateDereference((Expr.Dereference) expression, target, context);
		} else if (expression instanceof Expr.FieldDereference) {
			translateFieldDereference((Expr.FieldDereference) expression, target, context);
		} else if (expression instanceof Expr.Cast) {
			translateCast((Expr.Cast) expression, target, context);
	...
	// Translating "Expr.Dereference" expression into machine code
	public void translateDereference(Expr.Dereference e, Location target, Context context) {

	}

	// Translating "Expr.FieldDereference" expression into machine code
	public void translateFieldDereference(Expr.FieldDereference e, Location target, Context context) {

	}

	// Translating "Expr.Cast" expression into machine code
	public void translateCast(Expr.Cast e, Location target, Context context) {
	...
	// This method is inspired by the following source.
	// https://github.com/kianyewest/SWEN430/tree/master/swen430as4
	public void translateArrayAccess(Expr.ArrayAccess e, Location target, Context context) {
		Expr source = e.getSource();
		Expr index = e.getIndex();
		MemoryLocation elementLocation = indexHelper(source, index, context);
		bitwiseCopy(elementLocation, target, context);
	}

	// Creating index helper method which helps in dealing with indices.
	// This method is inspired by the following source.
	// https://github.com/kianyewest/SWEN430/tree/master/swen430as4
	public MemoryLocation indexHelper(Expr expression, Expr expressionIndex, Context context) {
		List<Instruction> instructions = context.instructions();
		// Translating source expression and placing it in a temporary register.
		RegisterLocation base = context.selectFreeRegister();
		translate(expression, base, context);
		// Locking base register to protected it
		context = context.lockLocation(base);
		// Translating index expression into another temporary register
		RegisterLocation index = context.selectFreeRegister();
		translate(expressionIndex, index, context);
		// Multiply index by WORD_SIZE while dealing with tag space
		multiplyByWordSize(index.register, context); // This will make my calculation for getting arr[n]
		// (nth element of array 'arr') become 16 + (n * 16).
		// Skipping the tag and adding the length field
		Instruction skipTagAndAddLengthField = new Instruction.ImmReg(Instruction.ImmRegOp.add, WORD_SIZE * 2L,
				index.register);
		instructions.add(skipTagAndAddLengthField);
		// Adding the index onto base register.
	...
	public void translateArrayGenerator(Expr.ArrayGenerator e, Location target, Context context) {
		...
		Expr arraySize = e.getSize();

		// Translating the size into machine code
		translate(arraySize, size, context);

		// Locking the register location of 'size' to ensure it is secure
		context = context.lockLocation(size);

		// Looking for a register location to put the array
		RegisterLocation array = context.selectFreeRegister();

		// Performing a bitwise copy from register location 'size' to register location 'array'
		bitwiseCopy(size, array, context);

		// Initialising instructions to be added to the list 'instructions'
		// 1. An instruction which deals with tags
		Instruction dealWithTags = new Instruction.ImmReg(Instruction.ImmRegOp.imul, 2, array.register);

		// 2. An instruction to increment size in order to consider the length part
		Instruction incrementSize = new Instruction.Reg(Instruction.RegOp.inc, array.register);

		// 3. An instruction to multiply by the amount of space each element in the newly initialised array takes
		Instruction multiplySize = new Instruction.ImmReg(Instruction.ImmRegOp.imul, WORD_SIZE, array.register);

		// 4. Creating a list containing instructions to be added and add them to the list 'instructions'
		List<Instruction> toBeAdded = Arrays.asList(dealWithTags, incrementSize, multiplySize);
		instructions.addAll(toBeAdded);

		// Allocating space for the array on heap
		allocateSpaceOnHeap(array, context);

		// Locking the register location of 'array' to prevent it from being overwritten.
		context = context.lockLocation(array);

		// Looking for memory location to put the length field
		MemoryLocation lengthField = new MemoryLocation(array.register, 0);

		// Creating a bitwise copy from memory location 'size' to 'lengthField'
		bitwiseCopy(size, lengthField, context);

		// Creating a bitwise copy from memory location 'array' to 'target'
		bitwiseCopy(array, target, context);

		// Looking for register location for the value to be written to a register
		RegisterLocation value = context.selectFreeRegister();
		Expr exprValue = e.getValue();

		// Translating the 'value' into machine code
		translate(exprValue, value, context);

		// Locking the register location of 'value' to prevent it from being overwritten.
		context = context.lockLocation(value);

		// Initialising an instruction to increment to a register location where we want to fill
		Instruction increment = new Instruction.ImmReg(Instruction.ImmRegOp.add, WORD_SIZE, array.register);

		// Adding the instruction 'increment' into 'instructions'
		instructions.add(increment);

		// Implementing filling
		Register arrayRegister = array.register;
		Register valueRegister = value.register;
		// Calling 'objfill'
		makeExternalMethodCall("objfill", context, null, arrayRegister, valueRegister);
	}
	...
	public void translateArrayInitialiser(Expr.ArrayInitialiser e, Location target, Context context) {
		// DONEFIXME: you will need to implement this
		// Getting the type of the expression 'e' and casting it into Type.Array
		Attribute.Type exprAttr = e.attribute(Attribute.Type.class);
		Type exprType = exprAttr.type;
		Type.Array arrayType = (Type.Array) unwrap(exprType);

		// Getting the number of arguments called in the array initialiser
		int numberOfArguments = e.getArguments().size();

		// List of arguments in the array initialiser
		List<Expr> argumentsList = e.getArguments();

		// Initialising register location for the uninitialised compound object
		RegisterLocation uninitialisedCompoundObject = compoundInitialiser(numberOfArguments, arrayType,
				target, context);

		// Locking the register location of 'uninitialisedCompoundObject' to prevent it from being overwritten.
		context = context.lockLocation(uninitialisedCompoundObject);

		// Translating all the arguments in the array initialiser into machine code. Then, write
		// them into heap space.
		int offsetSize = WORD_SIZE; // initial value
		int i = 0; // initial value
		while (i < numberOfArguments) {
			// Getting the argument at index 'i' in list 'argumentsList'
			Expr currArgument = argumentsList.get(i);

			// Allocating memory location for the payload to be written to
			MemoryLocation payload = new MemoryLocation(uninitialisedCompoundObject.register, offsetSize);

			// Getting the type of the current argument
			Attribute.Type argumentAttr = currArgument.attribute(Attribute.Type.class);
			Type argumentType = unwrap(argumentAttr.type);

			// Writing a value depending on whether the type of the current argument is a primitive or not
			boolean argumentIsPrimitive = isPrimitive(argumentType);
			translatePrimitiveLiteral(!argumentIsPrimitive, payload, context);

			// Proceed with the memory locations after payload
			MemoryLocation payloadField = new MemoryLocation(uninitialisedCompoundObject.register,
					offsetSize + WORD_SIZE);
			translate(currArgument, payloadField, context);
			offsetSize += 2 * WORD_SIZE;

			// Incrementing 'i' to move on to the next iteration of the while loop
			i++;
		}
	}
	...
	public void translateCompoundLiteral(List<Pair<Type,Object>> literals, Location target, Context context) {
		...
		for (int i = 0; i != literals.size(); ++i) {
			...
			// Allocating the memory location for the tag field
			MemoryLocation tagField = new MemoryLocation(base.register, offset);
			// Getting the type of the expression in 'ith' and then translate it depending on whether it is a
			// primitive or not
			Type exprType = ith.first();
			boolean exprIsPrimitive = isPrimitive(exprType);
			translatePrimitiveLiteral(!exprIsPrimitive, tagField, context);
			// Moving on to the payload
	...
	public void translateRecordConstructor(Expr.RecordConstructor e, Location target, Context context) {
		// Getting a list of fields in the record
		List<Pair<String, Expr>> recordFields = e.getFields();
		// Getting the type of the record object
		Type.Record recordType = (Type.Record) unwrap(e.attribute(Attribute.Type.class).type);
		// Constructing uninitialised compound object
		RegisterLocation base = compoundInitialiser(recordFields.size(), recordType, target, context);
		// Lock base to prevent it being overwritten
		context = context.lockLocation(base);
		// Translate fields in the order and write into the heap space
		int offset = WORD_SIZE;
		...
		for (int i = 0; i != recordFields.size(); ++i) {
			Pair<String, Expr> p = recordFields.get(i);
			// Initialising the memory location for the tag field of the record
			MemoryLocation tagField = new MemoryLocation(base.register, offset);
			// Getting the type of expression in the pair 'p'
			Expr expr = p.second();
			Attribute.Type exprAttr = expr.attribute(Attribute.Type.class);
			Type exprType = unwrap(exprAttr.type);
			// Translating 'exprType' depending on whether it is a primitive or not
			boolean exprIsPrimitive = isPrimitive(exprType);
			translatePrimitiveLiteral(!exprIsPrimitive, tagField, context);
			// Moving on to the payload
			offset = offset + WORD_SIZE;
			// Write payload
			MemoryLocation payloadField = new MemoryLocation(base.register, offset);
			translate(expr, payloadField, context);
			// Advance over payload
			offset += WORD_SIZE;
		}
	}
	...
	public void translateUnary(Expr.Unary e, RegisterLocation target, Context context) {
		...
		// 1. Getting a list of instructions in the context 'context'
		List<Instruction> instructions = context.instructions();

		// 2. Getting the expression in 'e' and then translating it into machine code.
		Expr expr = e.getExpr();
		translate(expr, target, context);

		// 3. Getting the unary operator in 'e'
		Expr.UOp unaryOperator = e.getOp();

		// 4. Adding instructions depending on the unary operator
		if (unaryOperator == Expr.UOp.NOT) {
			// Initialising instructions related to the 'NOT' unary operator
			// 1. The RegOp not
			Instruction implementNot = new Instruction.Reg(Instruction.RegOp.not, target.register);

			// 2. 'AND' operator
			Instruction implementAnd = new Instruction.ImmReg(Instruction.ImmRegOp.and, 1, target.register);

			// Adding the newly initialised instructions to the list 'instructions
			List<Instruction> toBeAdded = Arrays.asList(implementNot, implementAnd);
			instructions.addAll(toBeAdded);
		}
		else if (unaryOperator == Expr.UOp.NEG) {
			// Initialising an instruction to implement negation
			Instruction implementNegation = new Instruction.Reg(Instruction.RegOp.neg, target.register);
			instructions.add(implementNegation);
		}
		else if (unaryOperator == Expr.UOp.LENGTHOF) {
			// Initialising an instruction to get the length of an array
			Instruction getLengthOfArray = new Instruction.ImmIndReg(Instruction.ImmIndRegOp.mov, 0,
					target.register, target.register);
			instructions.add(getLengthOfArray);
		}
		else if (unaryOperator == Expr.UOp.NEW) {
			return;
		}
		else {
			// Throw an illegal argument exception
			throw new IllegalArgumentException("Unknown unary operator detected: " + unaryOperator);
		}
	}
	...
	public void translateVariable(Expr.Variable e, Location target, Context context) {
		...
		// Checking whether 'exprType' is primitive or not. If yes, implement a compound copy of 'exprType'.
		boolean exprIsPrimitive = isPrimitive(exprType);
		if (!exprIsPrimitive) {
	...
	public void compoundEquality(boolean positive, Location lhs, Location rhs, String targetLabel, Context context) {
		...
		// Call objcmp from runtime
		makeExternalMethodCall("objcmp", context, left.register, left.register, right.register);
	...
	private void extractLocalVariableTypes(List<Stmt> statements, Map<String, Type> allocation) {
		for (Stmt stmt : statements) {
			if (stmt instanceof Stmt.VariableDeclaration) {
				...
				Stmt.DoWhile fe = (Stmt.DoWhile) stmt;
				extractLocalVariableTypes(fe.getBody(), allocation);
	...
	public void multiplyByWordSize(Register register, Context context) {
		...
		// more efficient :)
		while (i < WORD_SIZE * 2) {
			instructions.add(new Instruction.Reg(Instruction.RegOp.shl, register)); // shifts to the left
			i = i << 1;
		}
	}
	...
	// The class 'Context' is inspired by the following source.
	// https://github.com/kianyewest/SWEN430/tree/master/swen430as4
	private class Context {
		...
		// A stack for break labels
		private final Stack<String> breakLabels;

		// A stack for continue labels
		...
		private Context(Context other) {
			...
			breakLabels = other.breakLabels;
			continueLabels = other.continueLabels;
		}
		...
		// Creating a method to add a break label
		public void addBreakLabel(String breakLabel) {
			breakLabels.push(breakLabel);
		}

		// Creating a method to generate a break label
		public String generateBreakLabel() {
			String toBeGenerated = freshLabel();
			addBreakLabel(toBeGenerated);
			return toBeGenerated;
		}

		// Creating a method to view most recent break label
		public String viewMostRecentBreakLabel() {
			return breakLabels.peek();
		}

		// Creating a method to get most recent break label
		public String getMostRecentBreakLabel() {
			return breakLabels.pop();
		}

		// Creating a method to add a continue label
		public void addContinueLabel(String continueLabel) {
			continueLabels.push(continueLabel);
		}

		// Creating a method to generate a continue label
		public String generateContinueLabel() {
			String toBeGenerated = freshLabel();
			addContinueLabel(toBeGenerated);
			return toBeGenerated;
		}

		// Creating a method to view most recent continue label
		public String viewMostRecentContinueLabel() {
			return continueLabels.peek();
		}

		// Creating a method to get most recent continue label
		public String getMostRecentContinueLabel() {
